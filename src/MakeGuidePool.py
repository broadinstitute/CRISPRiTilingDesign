...
def parseargs(required_args=True):
    ...
    parser.add_argument('--nGuidesPerElement', required=required_args, type=int, help="Number of guides to choose per guide set ('guideSet' column). Set to 0 to select all gRNAs.")
    ...
    parser.add_argument('--nCtrls', default=0, type=int, help="Number of negative control gRNAs to add to the pool")
    ...
    parser.add_argument('--nSafeCtrls', default=0, type=int, help="Number of safe-targeting gRNAs to add to the pool")
    ...
    parser.add_argument('--selectMethod', default='score', choices=['score','even'], help="Method to select guides within elements. 'even' selects every Nth gRNA evenly across the region [recommended]. 'score' selects by best off-target score [not recommended].")
